...
Run `npm run seed-data` - This will add automatically add:
...
1. To add tribes, run `npm run seed-data:tribes 50` — This will create 50 tribes.
    * Run this prior to adding users to add users to tribes automatically

2. To add users, run `npm run seed-data:users 1000 adminusername` — This will create 1000 users and hosting offers. `adminusername` is optional (a-z0-9) and will create an admin user.
    ...
    * Additional admin usernames are also supported (eg. `npm run seed-data:users 1000 admin1 admin2 admin3`)
    * If tribes exist, users will also be added to random tribes

3. To add messages, run `npm run seed-data:messages 1000 10` — This will create 1000 message threads between random users with up to 10 messages in each thread.
