...
For more custom setups, the scripts for generating data can be run individually. It is currently recommended that you run them in the sequence provided below.

1. To add tribes run `node scripts/db-seeding/fillTestTribesData.js 50` — This will create 50 tribes.
    * Run this prior to adding users to add users to tribes automatically

2. To add users run `node scripts/db-seeding/fillTestUsersData.js 1000 adminusername` — This will create 1000 users and hosting offers. `adminusername` is optional (a-z0-9) and will create an admin user.
    ...
    * Additional admin usernames are also supported (eg. `node scripts/db-seeding/fillTestUsersData.js 1000 admin1 admin2 admin3`)

3. To add messages run `node scripts/db-seeding/fillTestMessagesData.js 1000 10` — This will create 1000 message threads with up to 10 messages in each thread.
