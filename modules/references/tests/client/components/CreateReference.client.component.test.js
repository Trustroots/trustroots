...
describe('<CreateReference />', () => {
  ...
  it('should not be possible to leave a reference to self', () => {
    ...
    const { queryByRole } = render(<CreateReference userFrom={me} userTo={me} />);
    expect(queryByRole('alert')).toHaveTextContent('Sorry, you can\'t give a reference to yourself.');
  });

  it('check whether the reference exists at the beginning', () => {
    api.references.read.mockResolvedValueOnce([]);
    render(<CreateReference userFrom={userFrom} userTo={userTo}/>);
    expect(api.references.read).toBeCalledWith({ userFrom: userFrom._id, userTo: userTo._id });
  });

  it('can not leave a second reference', async () => {
    api.references.read.mockResolvedValueOnce([{ userFrom, userTo, public: false }]);
    const { queryByRole } = render(<CreateReference userFrom={userFrom} userTo={userTo}/>);
    await waitForLoader();
    expect(queryByRole('alert')).toHaveTextContent(`You've already given a reference to ${userTo.username}.`);
    expect(api.references.read).toBeCalledWith({ userFrom: userFrom._id, userTo: userTo._id });
  });

  it('can leave a reference (reference form is available)', async () => {
    api.references.read.mockResolvedValueOnce([]);
    const { queryByLabelText } = render(<CreateReference userFrom={userFrom} userTo={userTo}/>);
    await waitForLoader();
    for (const label of [
      'Met in person',
      'I hosted them',
  ...
  it('submit a reference', async () => {
    ...
    expect(queryByLabelText('Would you recommend others to stay with them?')).toBeInTheDocument();
  ...
  it('submit a report when recommend is no and user wants to send a report', async () => {
    ...
    fireEvent.change(getByLabelText('Message to moderators'), { target: { value: 'they were mean to me' } });
