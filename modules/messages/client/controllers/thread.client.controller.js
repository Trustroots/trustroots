...
/* @ngInject */
function MessagesThreadController(
  $rootScope,
  $scope,
  $stateParams,
  $state,
  ...
  if (
    ...
    !Authentication.user ||
    Authentication.user._id === userTo._id
  ) {
    $state.go('inbox');
  }
  ...
  const cachePrefix =
    'messages.thread.' + Authentication.user._id + '-' + $stateParams.username;
      ...
      function(error) {
        ...
        if (error.status === 404) {
          ...
          messageCenterService.add(
            'warning',
            error.message ||
              'Cannot load messages. Please refresh the page and try again.',
            { timeout: 20000 },
          );
        }
      },
  ...
  function fetchMessages() {
    return vm.messageHandler.fetchMessages({
      userId: userTo._id,
    });
  }
  ...
  function moreMessages() {
    if (vm.messageHandler.nextPage && !vm.messageHandler.paginationTimeout) {
      ...
      $analytics.eventTrack('thread-pagination', {
        ...
        label:
          'Message thread page ' +
          (vm.messageHandler.nextPage ? vm.messageHandler.nextPage.page : 0),
      });
    }
  }
  ...
  function messageRead(message, scrollingUp, scrollingDown) {
    ...
    if (
      message.userFrom._id !== Authentication.user._id &&
      !message.read &&
      read
    ) {
      message.read = true;
