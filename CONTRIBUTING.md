...
Jest treats the optional `canvas` package as absent so JSDOM uses its normal non-native canvas fallback. If non-test tooling fails while loading `canvas`, rebuild native dependencies for your current Node version with `npm rebuild canvas`. The project CI installs dependencies from scratch on Node 24, which avoids stale native binaries.
